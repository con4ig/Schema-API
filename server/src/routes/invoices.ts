 ...
 */
router.put("/archive", async (req: Request, res: Response): Promise<void> => {
  try {
    const { invoiceIds } = req.body as { invoiceIds?: string[] };

    if (!invoiceIds || !Array.isArray(invoiceIds) || invoiceIds.length === 0) {
      res.status(400).json({ msg: "No invoice IDs provided" });
      return;
    }
    ...
    res.json({
      message: `Archived ${result.modifiedCount} invoices`,
...
router.get("/export", async (_req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (invoices.length === 0) {
      res.status(404).json({ msg: "No approved invoices to export" });
      return;
    }
...
router.put(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
      const { vendor_name, date, total_net, total_gross, category } =
        req.body as InvoiceUpdateBody;
      ...
      if (Object.keys(req.body as object).length > 0) {
        ...
        if (total_net !== undefined && total_net !== "")
          updateFields.total_net = net;
        if (total_gross !== undefined && total_gross !== "")
          updateFields.total_gross = gross;
      }
      ...
      // Final verification check on the updated record
      const finalNet = Number(invoice.total_net);
      const finalGross = Number(invoice.total_gross);
      ...
      if (
        ...
        // Revert status if somehow it saved invalid data
...
router.put("/:id", async (req: Request, res: Response): Promise<void> => {
  try {
    const { vendor_name, date, total_net, total_gross, category, buyer_nip } =
      req.body as InvoiceUpdateBody;
